					  ...
					  int info[] , int nfev[] , double wa[] )
{
	...
	if ( info[0] != - 1 )
	{
		...
		if ( dginit >= 0 ) {
			wprintf(L"The search direction is not a descent direction.\n");
			return;
		}
					   ...
					   double xtol , int iflag[] ) //throw (ExceptionWithIflag* e)
{
	...
	while ( true )
	{
		...
		//wprintf(L">>L-BFGS.lbfgs Iteration: %d\n", iter );
					 ...
					 int m , double x[] , double f , double g[] , double stp[] , bool finish )
{
	...
	{
		wprintf(L"*************************************************\n");
		wprintf(L"  n = %d   number of corrections = %d\n       initial values\n", n, m);
		wprintf(L" f =  %f   gnorm =  %f\n", f, gnorm);
		if ( iprint [ 2 -1] >= 1 )
		{
			wprintf(L" vector x =");
			for ( i = 1; i <= n; i++ )
			...
			wprintf(L" gradient vector g =") ;
			for ( i = 1; i <= n; i++ )
				wprintf(L"  %f", g[i-1]);
			wprintf(L"\n");
		}
		wprintf(L"*************************************************\n");
		wprintf(L"\ti\tnfn\tfunc\tgnorm\tsteplength\n");
	}
	else
	{
		...
		if ( finish )
			wprintf(L" The minimization terminated without detecting errors. iflag = 0\n");
	}
	return;
}
