...
int MedSTC::train(char* start, char* directory, Corpus* pC, Params *param)
{
	...
	{
		printf("**** alternating minimization iteration %d ****\n", nIt + 1);

		// e-step
		printf("\t **** sparse coding ****\n");
		dobj = 0;
		double dLogLoss = 0;
		for ( int d=0; d<pC->num_docs; d++ ) {
			pDoc = &(pC->docs[d]);
	...
	for (int d=0; d<pC->num_docs; d++) {
		if ((d % 1000) == 0) printf("final sparse coding step Document %d\n",d);
	...
	fclose(w_asgn_file);
	fclose(lhood_file);
	printf("\n\n MedSTC: double count accuracy: %.5f\n\n", (double)nAcc / nNum);
